// Editor for smart_scripts: the SmartAI script attached to this object.
...
interface GameObjectSmartAiTabProps {
  goEntry: number;
}

export const GameObjectSmartAiTab: React.FC<GameObjectSmartAiTabProps> = ({ goEntry }) => {
  ...
  const loadScripts = async () => {
    ...
    try {
      const res = await api.executeSql(
        'world',
        `SELECT entryorguid, source_type, id, link, event_type, event_phase_mask, event_chance, event_flags, event_param1, event_param2, event_param3, event_param4, event_param5, event_param_string, action_type, action_param1, action_param2, action_param3, action_param4, action_param5, action_param6, target_type, target_param1, target_param2, target_param3, target_x, target_y, target_z, target_o, comment FROM \`smart_scripts\` WHERE \`entryorguid\` = ${goEntry} AND \`source_type\` = 1 ORDER BY \`id\` ASC;`
      );
      if (res && res.success && res.rows) {
        const loaded: SmartScriptRow[] = res.rows.map((r: any[]) => ({
          entryorguid: Number(r[0]),
          source_type: Number(r[1]) || 1,
          id: Number(r[2]) || 0,
          link: Number(r[3]) || 0,
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center py-20 text-slate-400">
        ...
        Loading GameObject SmartAI scripts...
      </div>
    );
  }
