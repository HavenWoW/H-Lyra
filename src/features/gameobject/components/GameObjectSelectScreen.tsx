...
interface GameObjectSelectScreenProps {
  ...
  setSearchScriptName: (val: string) => void;
  searchLimit: number;
  setSearchLimit: (val: number) => void;
  searchResults: GameObjectSearchRow[] | null;
  searching: boolean;
  onSearch: () => void;
  onSelectExistingGameObject: (entry: number) => void;
  onCreateNewGameObject: (entry: number) => void;
  getSearchQueryInfo: () => { sql: string; whereSql: string; limit: number };
}

export const GameObjectSelectScreen: React.FC<GameObjectSelectScreenProps> = ({
  searchEntry,
  setSearchEntry,
  searchName,
  setSearchName,
  ...
  searching,
  onSearch,
  onSelectExistingGameObject,
  onCreateNewGameObject,
  getSearchQueryInfo,
}) => {
  const [sortKey, setSortKey] = useState<GameObjectSortKey>('entry');
  const [sortAsc, setSortAsc] = useState(true);
  const [currentPage, setCurrentPage] = useState(1);
  const rowsPerPage = 20;

  const handleSort = (key: GameObjectSortKey) => {
    if (sortKey === key) {
      setSortAsc(!sortAsc);
    } else {
      setSortKey(key);
  ...
  const renderSortHeader = (label: string, field: GameObjectSortKey, width?: string, align: 'left' | 'center' = 'left') => {
  ...
  return (
    <div className="flex-1 flex flex-col h-full bg-[#F0F2F5] overflow-y-auto p-4 space-y-4 select-none font-sans">
      {/* Card 1: Create New */}
      <EntityCreateCard
        entityTable="gameobject_template"
        entityIdField="entry"
        customStartingId={900000}
        onSelect={onCreateNewGameObject}
      />
      ...
      <div className="bg-white border border-[#E2E8F0] rounded p-4 space-y-3 shadow-sm flex-1 flex flex-col">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <input
            ...
            onKeyDown={(e) => { if (e.key === 'Enter') onSearch(); }}
            placeholder="Name"
            className="w-48 px-2.5 py-1.5 text-xs border border-slate-300 rounded focus:border-blue-500 focus:outline-none placeholder:text-slate-400 font-sans"
          />
          <select
            value={searchType}
            onChange={(e) => setSearchType(e.target.value)}
            className="w-40 px-2.5 py-1.5 text-xs border border-slate-300 rounded focus:border-blue-500 focus:outline-none font-sans text-slate-700"
          >
            <option value="">Select Type</option>
            {GAMEOBJECT_TYPE_OPTIONS.map((o) => (
          ...
          <input
            ...
            className="w-40 px-2.5 py-1.5 text-xs border border-slate-300 rounded focus:border-blue-500 focus:outline-none placeholder:text-slate-400 font-sans font-mono"
          />
          <input
            type="number"
            value={searchLimit}
        ...
        <div className="bg-[#F8F9FA] border border-slate-200 rounded p-2.5 text-xs font-mono text-slate-700 break-all select-text">
          <span className="text-[#0D6EFD] font-bold">SELECT</span> `entry`, `name`, `type`, `displayId`, `AIName`, `ScriptName` <span className="text-[#0D6EFD] font-bold">FROM</span> `gameobject_template`
          {qInfo.whereSql && (
            <>
              ...
              <div className="flex-1 overflow-auto">
                <table className="w-full text-[13.5px] text-left border-collapse font-sans table-fixed">
                  <thead className="bg-[#F8FAFC] border-b border-[#CBD5E1] text-slate-700 font-semibold sticky top-0 select-none text-[13px]">
                    <tr>
                      {renderSortHeader('Entry', 'entry', '10%', 'center')}
                      {renderSortHeader('Name', 'name', '25%', 'left')}
                      {renderSortHeader('Type', 'typeName', '15%', 'center')}
                      {renderSortHeader('Display Id', 'displayId', '10%', 'center')}
                      {renderSortHeader('Type', 'type', '8%', 'center')}
                      {renderSortHeader('AIName', 'AIName', '15%', 'left')}
                      {renderSortHeader('ScriptName', 'ScriptName', '17%', 'left')}
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-[#E2E8F0] bg-white">
                    {currentRows.map((row) => (
                      <tr
                        key={row.entry}
                        onClick={() => onSelectExistingGameObject(row.entry)}
                      ...
                      >
                        ...
                        <td className="py-2.5 px-3 font-mono text-slate-700 text-center text-[12.5px] truncate font-semibold">
                          {GAMEOBJECT_TYPE_OPTIONS.find((o) => o.value === row.type)?.name ||
                            `TYPE_${row.type}`}
                        </td>
                        ...
                        <td className="py-2.5 px-3 text-slate-600 text-left text-[13px] truncate font-mono">
                          {row.AIName || ''}
                        </td>
                        <td className="py-2.5 px-3 text-slate-600 text-left text-[13px] truncate font-mono">
                          {row.ScriptName || ''}
