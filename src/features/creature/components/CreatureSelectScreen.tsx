// Creature search and selection, and the entry point for creating a new one.

import React, { useState, useMemo } from 'react';
import { Search, ChevronUp, ChevronDown, ChevronsUpDown } from 'lucide-react';
import { EntityCreateCard } from '../../../components/EntityCreateCard';
import { CreatureSearchRow, CreatureSortKey } from '../types';

interface CreatureSelectScreenProps {
  searchEntry: string;
  setSearchEntry: (val: string) => void;
  searchName: string;
  setSearchName: (val: string) => void;
  searchSubname: string;
  setSearchSubname: (val: string) => void;
  searchLimit: number;
  setSearchLimit: (val: number) => void;
  searchResults: CreatureSearchRow[] | null;
  searching: boolean;
  onSearch: () => void;
  onSelectExistingCreature: (entry: number) => void;
  onCreateNewCreature: (entry: number) => void;
  getSearchQueryInfo: () => { sql: string; whereSql: string; limit: number };
}

export const CreatureSelectScreen: React.FC<CreatureSelectScreenProps> = ({
  searchEntry,
  setSearchEntry,
  searchName,
  setSearchName,
  ...
  searching,
  onSearch,
  onSelectExistingCreature,
  onCreateNewCreature,
  getSearchQueryInfo,
}) => {
  const [sortKey, setSortKey] = useState<CreatureSortKey>('entry');
  const [sortAsc, setSortAsc] = useState(true);
  const [currentPage, setCurrentPage] = useState(1);
  const rowsPerPage = 20;

  const handleSort = (key: CreatureSortKey) => {
    if (sortKey === key) {
      setSortAsc(!sortAsc);
    } else {
      setSortKey(key);
  ...
  const renderSortHeader = (label: string, field: CreatureSortKey, width?: string, align: 'left' | 'center' = 'left') => {
  ...
  return (
    <div className="flex-1 flex flex-col h-full bg-[#F0F2F5] overflow-y-auto p-4 space-y-4 select-none font-sans">
      {/* Card 1: Create New */}
      <EntityCreateCard
        entityTable="creature_template"
        entityIdField="entry"
        customStartingId={200000}
        onSelect={onCreateNewCreature}
      />
      ...
      <div className="bg-white border border-[#E2E8F0] rounded p-4 space-y-3 shadow-sm flex-1 flex flex-col">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <input
            type="text"
            value={searchSubname}
            onChange={(e) => setSearchSubname(e.target.value)}
            onKeyDown={(e) => { if (e.key === 'Enter') onSearch(); }}
            placeholder="SubName / Title"
        ...
        <div className="bg-[#F8F9FA] border border-slate-200 rounded p-2.5 text-xs font-mono text-slate-700 break-all select-text">
          <span className="text-[#0D6EFD] font-bold">SELECT</span> `entry`, `name`, `subname`, `minlevel`, `maxlevel`, `AIName`, `ScriptName` <span className="text-[#0D6EFD] font-bold">FROM</span> `creature_template`
          {qInfo.whereSql && (
            <>
                ...
                <table className="w-full text-[13.5px] text-left border-collapse font-sans table-fixed">
                  <thead className="bg-[#F8FAFC] border-b border-[#CBD5E1] text-slate-700 font-semibold sticky top-0 select-none text-[13px]">
                    <tr>
                      ...
                      {renderSortHeader('name', 'name', '22%', 'left')}
                      {renderSortHeader('subname', 'subname', '18%', 'left')}
                      {renderSortHeader('minlevel', 'minlevel', '10%', 'center')}
                      {renderSortHeader('maxlevel', 'maxlevel', '10%', 'center')}
                      {renderSortHeader('AIName', 'AIName', '15%', 'center')}
                      {renderSortHeader('ScriptName', 'ScriptName', '15%', 'center')}
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-[#E2E8F0] bg-white">
                    {currentRows.map((row) => (
                      <tr
                        key={row.entry}
                        onClick={() => onSelectExistingCreature(row.entry)}
                      ...
                      >
                        ...
                        <td className="py-2.5 px-3 font-mono text-slate-600 text-center text-[13px] truncate">
                          {row.AIName || ''}
                        </td>
                        <td className="py-2.5 px-3 font-mono text-slate-600 text-center text-[13px] truncate">
                          {row.ScriptName || ''}
